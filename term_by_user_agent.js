...
var actions = [
    ...
    () => {
      // edit Breakdown
      $('fieldset').has('legend:contains("Breakdown")').find('button[title^="Edit"][title$="configuration"], button[aria-label="Add or drag-and-drop a field to Breakdown"]').click();
      },
    () => {
    ...
    () => {
      const inputElement = $('div[data-test-subj="indexPattern-dimension-field"]').find('input')[0];
      setInputValue(inputElement, "user_agent.keyword");
      },
    () => {
      // click client_ip in drop down
      $(".euiPopover__panel").find('button.euiFilterSelectItem[title="user_agent.keyword"]').click();
      },
    () => {
